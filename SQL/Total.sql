...
AS
    BEGIN
        ...
        IF EXISTS ( -- Nếu người dùng đang mượn phòng học
            ...
            WHERE MPH._ReturnAt IS NULL
            GROUP BY i.idNguoiMuonPhong
            HAVING COUNT(*) > 1
        )
        BEGIN -- Thông báo lỗi và rollback
            DECLARE @idLichMuonPhong VARCHAR(50) = (SELECT CAST(i.[idLichMuonPhong] AS VARCHAR(50)) FROM inserted i)
            RAISERROR('Cannot insert or update: NguoiDung is currently borrowing a room. idLichMuonPhong = %s', 16, 1, @idLichMuonPhong)
            ROLLBACK TRANSACTION
            RETURN
        END
...
AS
    BEGIN
        ...
        IF EXISTS ( -- Nếu thời gian bắt đầu và kết thúc của lịch mượn phòng trùng với lịch mượn phòng khác trong cùng một nhóm tổ học phần
            SELECT 1
            FROM inserted AS i
            INNER JOIN [dbo].[LichMuonPhong] AS LMP ON i.idNhomToHocPhan = LMP.idNhomToHocPhan
        ...
        BEGIN -- Thông báo cảnh báo
            DECLARE @idLichMuonPhong VARCHAR(50) = (SELECT CAST(i.[idLichMuonPhong] AS VARCHAR(50)) FROM inserted i)
            RAISERROR('Warning!: LichMuonPhong.startDateTime and LichMuonPhong.endDateTime should not overlap with another LichMuonPhong in the same NhomToHocPhan. idLichMuonPhong = %s', 10, 1, @idLichMuonPhong)
        END
    END
GO

-- Ràng buộc thời gian nhóm tổ học phần trong khoảng thời gian của học kỳ
-- Ràng buộc nhóm tổ phù hợp với mục đích sử dụng
...
AS
    BEGIN
        ...
        IF NOT EXISTS ( -- Nếu nhóm tổ học phần không phù hợp với mục đích sử dụng
            SELECT 1
            FROM inserted AS i
            WHERE i.nhomTo = 0 AND i.mucDich = 'LT'
                OR i.nhomTo <> 0 AND i.mucDich = 'TH'
        )
        BEGIN -- Thông báo lỗi và rollback
            DECLARE @idNhomToHocPhan2 VARCHAR(50) = (SELECT CAST(i.[idNhomToHocPhan] AS VARCHAR(50)) FROM inserted i)
            RAISERROR ('Cannot insert or update: NhomToHocPhan.nhomTo must be 0 with mucDich = ''LT'' or NhomToHocPhan.nhomTo must be not 0 with mucDich = ''TH''. idNhomToHocPhan = %s', 16, 1, @idNhomToHocPhan2)
            ROLLBACK TRANSACTION
        END
    END
GO
...
AS
    BEGIN
        ...
        IF EXISTS ( -- Nếu nhóm học phần không duy nhất với nhóm tổ
            SELECT 1
            FROM inserted AS i
